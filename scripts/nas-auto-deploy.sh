...
# 3. 서버 재시작 — 부모 세션과 완전 분리(setsid + nohup + 새 세션)
log "서버 재시작 중..."
pkill -f 'python3 server.py' 2>>"$LOG"
sleep 3
# 이중 fork 패턴: setsid + nohup + 명시적 redirect로 SIGHUP 절연
nohup setsid python3 "$APP_DIR/server.py" </dev/null >>"$APP_DIR/server.log" 2>&1 &
