...
var DB={
  ...
  getConflict:function(k){
    var storeKey=k&&k.indexOf('ino_')===0?k:'ino_'+k;
    return _conflicts[storeKey]||null;
  },
  clearConflict:function(k){
    var storeKey=k&&k.indexOf('ino_')===0?k:'ino_'+k;
    delete _conflicts[storeKey];
  },
  retryConflict:function(k,useLocal){
    var storeKey=k&&k.indexOf('ino_')===0?k:'ino_'+k;
    var conflict=_conflicts[storeKey];
    if(!conflict)return false;
    if(useLocal){
      _cache[storeKey]=conflict.localValue;
      localStorage.setItem(storeKey,conflict.localValue);
      _meta[storeKey]=conflict.updatedAt||null;
...
function _conflictLabel(storeKey){
  var m={
    'ino_cli':'거래처',
    'ino_prod':'품목',
    'ino_orders':'수주',
  ...
  return m[storeKey]||storeKey.replace(/^ino_/,'');
}
function _conflictPretty(value){
...
function openConflictMo(storeKey){
  var conflict=DB.getConflict(storeKey);
  if(!conflict)return;
  window._activeConflictKey=storeKey;
  $('conflictKeyLabel').textContent=_conflictLabel(storeKey);
  $('conflictUpdatedAt').textContent=conflict.updatedAt||'-';
  $('conflictServerValue').textContent=_conflictPretty(conflict.serverValue);
  $('conflictLocalValue').textContent=_conflictPretty(conflict.localValue);
  oMo('conflictMo');
}
function keepServerConflict(){
  if(!window._activeConflictKey)return;
  DB.clearConflict(window._activeConflictKey);
  cMo('conflictMo');
  toast('서버 최신값을 유지했습니다','ok');
}
function retryConflictSave(){
  if(!window._activeConflictKey)return;
  var key=window._activeConflictKey;
  DB.retryConflict(key,true);
  cMo('conflictMo');
  toast('내 변경을 최신 서버 기준으로 다시 저장했습니다','ok');
}
window.addEventListener('packflow-db-conflict',function(e){
  if(e&&e.detail&&e.detail.key)openConflictMo(e.detail.key);
});
...
async function unifiedLogin(){var user=$('loginUser').value,pw=$('loginPw').value;if(!user){$('loginErr').textContent='사용자를 선택하세요';return}
  ...
  /* 기본 비밀번호 1234 경고 — 사용자 요청으로 비활성화됨 (테스트 환경) */
}
...
function _onWindowResize(){
  ...
  _resizeTimer=setTimeout(function(){
    ...
    if(['mes-dash','mes-rpt','mes-ship','mes-wo','acc-cashflow'].indexOf(mod)>-1){
      try{
...
var PG={'mes-order':'mes-admin','mes-shiplog':'mes-admin','mes-dash':'mes-admin','mes-wo':'mes-admin','mes-ship':'mes-admin','mes-cli':'mes-admin','mes-prod':'mes-admin','mes-vendor':'mes-admin','mes-mold':'mes-admin','mes-rpt':'mes-admin','mes-plan':'mes-admin','mes-queue':'mes-admin','mes-worker':'mes-admin','mes-perf':'mes-admin','mes-cal':'mes-admin','mat-stock':'mat-income','mat-po':'mat-income','mat-bom':'mat-income','acc-purchase':'acc-sales','acc-tax':'acc-sales','qc-equip':'qc-inspect','qc-quote':'qc-inspect'};
var TAB_MAP={'mat-income':'income','mat-stock':'stock','mat-po':'po','mat-bom':'bom','acc-sales':'sales','acc-purchase':'purchase','acc-tax':'tax','qc-inspect':'qc','qc-equip':'equip','qc-quote':'quote'};
var GROUPS={
...
'mes-dash':{tabs:[{id:'mes-dash',l:'생산 현황'},{id:'mes-worker',l:'작업 대기열'}]},
...
'mat-bom':{tabs:[{id:'mat-bom',l:'BOM/소요자재'}]},
...
'acc-recv':{tabs:[{id:'acc-recv',l:'미수·미지급'},{id:'acc-cashflow',l:'입출금'}]},
'mes-cli':{tabs:[{id:'mes-cli',l:'거래처'},{id:'mes-vendor',l:'외주처'},{id:'mes-prod',l:'품목'},{id:'mes-mold',l:'목형'},{id:'adm-code',l:'공통 코드'}]},
'adm-backup':{tabs:[{id:'adm-backup',l:'백업/복원'},{id:'adm-audit',l:'감사 로그'},{id:'mes-queue',l:'시스템 설정'}]}
};
...
var titleMap={'mes-order':'수주관리','mes-shiplog':'출고내역','mes-dash':'생산 현황','mes-wo':'작업지시서','mes-ship':'출고','mes-cli':'거래처','mes-prod':'품목','mes-mold':'목형','mes-rpt':'생산보고','mes-cal':'캘린더','mes-sched':'스케줄 보드','mes-perf':'성과분석','mes-plan':'생산계획','mes-vendor':'외주처','mes-queue':'시스템 설정','mes-worker':'작업 대기열','mat-income':'입고','mat-stock':'자재 재고','mat-po':'발주서','mat-bom':'BOM/소요자재','acc-sales':'매출','acc-purchase':'매입','acc-tax':'세금계산서','acc-recv':'미수·미지급','acc-cashflow':'입출금','qc-inspect':'품질검사','qc-equip':'설비','qc-quote':'패키지 견적','mes-order-track':'납기추적','mes-due':'납기관리','mes-proc-log':'공정실적','mes-lot':'로트추적','mes-downtime':'비가동관리','mes-defect':'불량관리','mes-outsource':'외주 현황','mes-mold-hist':'금형이력','mes-oee':'OEE','ship-partial':'부분출고','ship-return':'반품관리','ship-inspect':'출하검사','mat-safety':'안전재고','mat-mrp':'MRP','mat-audit':'재고실사','mat-price':'단가이력','acc-etax':'전자세금계산서','acc-costing':'원가분석','acc-aging':'채권 Aging','qc-claim':'클레임','qc-pm':'예방보전','qc-breakdown':'고장이력','qc-cert':'검사성적서','adm-perm':'권한 관리','adm-backup':'백업/복원','adm-audit':'감사 로그','adm-code':'공통 코드','mes-closing':'월말 마감 점검'};
if(titleMap[id]){var mt=$('mainTitle');if(mt)mt.textContent=titleMap[id];var bc=$('mainBreadcrumb');if(bc){var bcMap={'mes-order':'견적·수주','mes-order-track':'견적·수주','qc-quote':'견적·수주','qc-claim':'출고·매출','mes-wo':'생산·공정','mes-dash':'패키지 제조','mes-plan':'생산·공정','mes-worker':'생산·공정','mes-proc-log':'생산·공정','mes-lot':'생산·공정','mes-outsource':'생산·공정','mat-stock':'자재·구매','mat-income':'자재·구매','mat-safety':'자재·구매','mat-audit':'자재·구매','mat-po':'자재·구매','mat-price':'자재·구매','mat-bom':'자재·구매','mat-mrp':'자재·구매','mes-ship':'출고·매출','ship-partial':'출고·매출','ship-return':'출고·매출','ship-inspect':'출고·매출','qc-inspect':'기초정보','qc-cert':'출고·매출','mes-defect':'출고·매출','acc-sales':'출고·매출','acc-purchase':'출고·매출','acc-tax':'출고·매출','acc-costing':'출고·매출','acc-etax':'출고·매출','acc-recv':'출고·매출','acc-cashflow':'출고·매출','mes-cli':'기초정보','mes-vendor':'자재·구매','mes-prod':'기초정보','mes-mold':'기초정보','qc-equip':'기초정보','qc-pm':'기초정보','qc-breakdown':'기초정보','mes-downtime':'기초정보','mes-oee':'기초정보','mes-mold-hist':'기초정보','adm-code':'시스템','adm-perm':'시스템','adm-backup':'시스템','adm-audit':'시스템','mes-queue':'시스템','mes-closing':'출고·매출'};if(bcMap[id])bc.textContent=bcMap[id];else bc.textContent='패키지 제조'}var sub=$('mainSubBc');if(sub)sub.textContent=titleMap[id]||''}
...
ER['qc-inspect']=function(){if(typeof rQc==='function')rQc()};ER['qc-equip']=function(){if(typeof rEq==='function'){rEq();renderEqUtil()}};ER['qc-quote']=function(){if(typeof rQt==='function')rQt()};
...
(function(){
  ...
  if(typeof DB !== 'undefined' && DB.s && !DB.s.__syncWrapped){
    ...
    var GUARDED_KEYS = ['cli','prod','mold','vendors','users','co','wo','sales','purchase','taxInvoice','shipLog','hist','income','stock','orders','claims'];
    DB.s = function(k, d){
